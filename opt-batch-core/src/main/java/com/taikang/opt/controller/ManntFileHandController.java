...
@RestController
@RequestMapping(value = "ManntFile")
public class ManntFileHandController {
    @Autowired
    private JobLauncher jobLauncher;

    @Autowired
    @Qualifier(value ="updateManntFileJob" )
    private Job job;
    ...
    public ResponseData hand(){
        ...
        try {
            ...
            return ResponseData.ok().data("updateManntFileJob--跑批成功");
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseData.error().data("updateManntFileJob--跑批失败");
        }
    }
}

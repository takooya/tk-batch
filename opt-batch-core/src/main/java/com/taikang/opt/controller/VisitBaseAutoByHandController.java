 ...
 */
@RestController
@RequestMapping(value = "visitBaseAuto")
public class VisitBaseAutoByHandController {
    @Autowired
    private JobLauncher jobLauncher;
    @Autowired
    @Qualifier("visitBaseJobAuto")
    private Job job;
   ...
   @GetMapping
    public ResponseData hand(){
        ...
        try {
            ...
            return ResponseData.ok().data("visitBaseAuto自动跑批成功");
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseData.error().data("visitBaseAuto自动跑批失败"+e.getMessage());
        }
    }
}
